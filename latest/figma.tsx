...
export default function UserStoriesTab({ goTogerkin, userstoriesData, fulluserstoriesdataPayload, fullValidatorPayload, onUpdatedUserStoriesData, onUpdateUserStoriesPayload }: UserStoriesProps) {
  // Moved requirements state to the top
  ...
  }, [confidenceThreshold, roleFilter, selectedIds, searchText, tagFilter]);
  ...
  // Fixed useEffect to handle one-to-many properly
  useEffect(() => {
    ...
    // Group stories by requirement_id to handle one-to-many properly
    const groupedByRequirement = userstoriesData.reduce((acc, storyObj) => {
      if (!acc[storyObj.requirement_id]) {
        acc[storyObj.requirement_id] = {
      ...
      acc[storyObj.requirement_id].stories.push({
      ...
      return acc;
    }, {} as Record<string, FormattedRequirement>);

    const formattedData = Object.values(groupedByRequirement);
  ...
  // Memoized unique roles and tags
  ...
  }, [requirements, confidenceThreshold, roleFilter, selectedIds, searchText, tagFilter]);

  // Memoized pagination calculations
  const totalPages = useMemo(() => {
    return Math.ceil(filteredRequirements.length / rowsPerPage);
  }, [filteredRequirements.length, rowsPerPage]);

  const paginated = useMemo(() => {
    const startIndex = (currentPage - 1) * rowsPerPage;
    return filteredRequirements.slice(startIndex, startIndex + rowsPerPage);
  }, [filteredRequirements, currentPage, rowsPerPage]);
  ...
  // Fixed toggleExpand to use req.id
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      // Group regenerated data properly
      const groupedByRequirement = regenerateData.reduce((acc, storyObj) => {
        if (!acc[storyObj.requirement_id]) {
          acc[storyObj.requirement_id] = {
            id: storyObj.requirement_id,
            confidence: storyObj.confidence_score ?? 0.5,
  ...
  // Fixed expandAll useEffect to use req.id
  useEffect(() => {
    const newStates: Record<string, boolean> = {};
    requirements.forEach((r) => {
      newStates[r.id] = expandAll; // Use req.id instead of story.usid
    });
 ...
 const handleEditSave = () => {
  ...
  // Update the selected story in the requirements state
      ...
      req.id === currentReqId
        ? {
            ...
            stories: req.stories.map((s) =>
              s.usid === selectedStory?.usid ? normalizedStory : s
            ),
            // Update confidence only if needed (e.g., average or max of stories' confidence)
            confidence: Math.max(
              ...req.stories.map((s) =>
                s.usid === selectedStory?.usid ? normalizedConfidence : s.confidence
              )
            ),
          }
        : req
  ...
  // Update only the specific user story in fulluserstoriesdataPayload
  const updatedStories = fulluserstoriesdataPayload.user_stories.map((r: Requirement) =>
    r.requirement_id === currentReqId && r.user_story_id === selectedStory?.usid
      ? {
          ...r,
      ...
      <div className="bg-[#f6f6f6] dark:bg-[#181818] rounded-2xl text-black dark:text-white h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="flex gap-4 p-6 items-center">
          ...
          <Popover>
            ...
            <PopoverContent className="w-56 p-2 bg-gray-900 border-gray-700 text-white">
        ...
        <ScrollArea className="flex overflow-y-auto px-4 pr-6 pb-4">
          {paginated.map((req) => (
            <div key={req.id} className="rounded-2xl border dark:border-gray-700 mb-4 ml-2 mr-2">
              <button
                onClick={() => toggleExpand(req.id)} // Fixed: use req.id
              ...
              >
                ...
                <div className="flex items-center gap-2">
                 <div className="text-sm text-gray-700 dark:text-gray-400">Story Id – <span className="text-black dark:text-white">{req.stories.map(story => story.usid).join(', ')}</span></div>
                  ...
                  {expandedReqs[req.id] ? ( // Fixed: use req.id
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                value={rowsPerPage}
                onChange={(e) => {
                  setCurrentPage(1); // Fixed: Reset to page 1 when changing rows per page
    ...
    <DialogHeader className="sticky top-0 bg-[#1a1a1a] z-10 border-b border-gray-700 p-4">
      <DialogTitle className="text-sm text-gray-400 flex justify-between items-center">
        {viewMode === 'refine' && updatedStory ? (
          ...
          <>
            {viewMode === 'refine' ? 'Feedback & Refine' : 'Edit/Feedback & Refine'}
                ...
                <div className="flex-1 overflow-y-auto space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      disabled
                      className="w-full rounded-md border border-gray-700  p-2 text-gray-400 cursor-not-allowed"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Title"
                      className="w-full rounded-md border border-gray-700  p-2 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full rounded-md border border-gray-700  p-2 text-white h-24 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full rounded-md border border-gray-700  p-2 text-white h-32 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div>
