...
export default function FigmaUserStoriesTab({ 
  ...
  const handleRegenerate = async () => {
    if (!uploadedImageFile) {
      alert("No image file available for regeneration. Please upload an image first.");
      return;
    }
    ...
    try {
      ...
      if (uploadedDocFile) {
        formData.append('reference_files', uploadedDocFile);
      }
      ...
      console.log("Regenerated data:", JSON.stringify(result, null, 2));

      const mappedStories = result.user_stories
      ...
      onUpdateUserStoriesPayload(result);
      onUpdatedUserStoriesData(mappedStories);

      setShowMessage(true);
      setTimeout(() => setShowMessage(false), 3000);
    } catch (err) {
      console.error("Regeneration error:", err);
      alert(`Failed to regenerate user stories: ${err instanceof Error ? err.message : 'Unknown error'}`);
    } finally {
      ...
      <div className="bg-[#f6f6f6] dark:bg-[#181818] rounded-2xl text-black dark:text-white h-[75vh] overflow-hidden flex flex-col">
        ...
        <div className="flex justify-between gap-2 mt-2">
          <div className="min-w-[300px] h-[30px] flex items-center">
            {showMessage && (
              <span className="flex flex-row gap-2">
                ...
                User stories regenerated successfully
              </span>
            )}
            {showPostingSuccess && (
              <span className="flex flex-row gap-2">
          ...
          <div className="flex gap-1">
            ...
            <Button
              className="bg-white dark:bg-[#0D0D0D] text-black dark:text-white border border-gray-400"
              disabled={filteredRequirements.length === 0 || isRegenerating || !uploadedImageFile}
    ...
    <DialogHeader className="sticky top-0 bg-[#1a1a1a] z-10 border-b border-gray-700 p-4">
      <DialogTitle className="text-sm text-gray-400 flex justify-between items-center">
        {viewMode === 'refine' && updatedStory ? (
          ...
          <>
            {viewMode === 'refine' ? 'Feedback & Refine' : 'Edit/Refine & Feedback'}
...
{viewMode === 'refine' && (
  ...
  />
    ...
    <div className="border-t border-gray-700 pt-2">
      ...
      <div className="p-2 rounded-md" style={{ backgroundColor: '#2E2E2E' }}>
  ...
  <div className="relative mb-2">
    ...
    <button
      ...
      className={`absolute right-8 top-1/2 -translate-y-1/2 p-1.5 rounded-full z-10 ${
    ...
    <button
      ...
      className={`absolute right-1 top-1/2 -translate-y-1/2 p-1.5 rounded-full z-10 bg-purple-600 text-white transition duration-200 ease-in-out
