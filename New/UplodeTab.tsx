...
const UploadTab = ({
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      const requirements = uploadType === 'document' ? data.extracted_requirements : data.converted_requirements;
      if (!requirements) {
        throw new Error(`No requirements found in response under ${uploadType === 'document' ? 'extracted_requirements' : 'converted_requirements'}`);
      }
